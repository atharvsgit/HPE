...
GRANT SELECT, INSERT, UPDATE ON dq_config.dq_rules TO dq_app;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA dq_config TO dq_app;

GRANT USAGE ON SCHEMA dq_results TO dq_app;
GRANT SELECT ON dq_results.test_results TO dq_app;
...
ALTER DEFAULT PRIVILEGES IN SCHEMA dq_config GRANT SELECT, INSERT, UPDATE ON TABLES TO dq_app;
