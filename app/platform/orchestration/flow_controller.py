    ...
    async with metadata_engine.begin() as conn:
        result = await conn.execute(
            text("""
                ...
                VALUES
                    (:run_id, :table_name, :row_count, :column_count,
                     :null_summary::jsonb, :schema_info::jsonb,
                     :statistics::jsonb, :uniqueness::jsonb, :profiled_at)
            ...
            """),
            {
                ...
                "profiled_at": profile.get("profiled_at", datetime.now(UTC).isoformat()),
            },
        )
    ...
    async with metadata_engine.begin() as conn:
        await conn.execute(
            text("""
                ...
                VALUES
                    (:run_id, :table_name, :column_name, :method,
                     :anomaly_count, :total_rows, :anomaly_pct, :details::jsonb)
            """),
            {
                "run_id": run_id,
