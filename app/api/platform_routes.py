...
Endpoints:
    ...
    GET  /platform/pipeline/runs/{run_id}/events — get run execution events
    POST /platform/pipeline/schedules        — create recurring pipeline schedule
    GET  /platform/pipeline/schedules        — list recurring pipeline schedules
    PATCH /platform/pipeline/schedules/{id}  — enable/disable a schedule
...
from app.platform.orchestration.task_scheduler import (
...
async def trigger_pipeline(
    ...
    log.info(
        "Pipeline triggered: run_id={r}, table='{t}'", r=run_id, t=request.table_name
    )
...
@platform_router.get(
    "/pipeline/runs/{run_id}/events",
    response_model=list[PipelineEventResponse],
    summary="List execution events for one pipeline run",
)
async def list_pipeline_run_events(run_id: int) -> list[PipelineEventResponse]:
    async with metadata_engine.connect() as conn:
        result = await conn.execute(
            text(
                """
                SELECT event_id, run_id, stage, level, message, details, created_at
                FROM dq_platform.pipeline_events
                WHERE run_id = :run_id
                ORDER BY event_id
                """
            ),
    ...
    return [_pipeline_event_from_row(row) for row in rows]
...
@platform_router.post(
    ...
    summary="Create a recurring platform pipeline schedule",
)
async def create_schedule(
    request: PipelineScheduleCreateRequest,
) -> PipelineScheduleResponse:
    try:
        schedule = await create_pipeline_schedule(
            table_name=request.table_name,
            schedule_cron=request.schedule_cron,
            is_enabled=request.is_enabled,
            description=request.description,
        )
    except (CronValidationError, SourceDataAccessError) as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@platform_router.get(
    "/pipeline/schedules",
    response_model=list[PipelineScheduleResponse],
    summary="List recurring platform pipeline schedules",
)
async def list_schedules() -> list[PipelineScheduleResponse]:
...
@platform_router.patch(
    ...
    response_model=PipelineScheduleResponse,
    summary="Enable or disable a recurring platform pipeline schedule",
)
async def update_schedule_status(
    schedule_id: int,
    request: PipelineScheduleUpdateRequest,
) -> PipelineScheduleResponse:
    schedule = await set_pipeline_schedule_enabled(schedule_id, request.is_enabled)
    if schedule is None:
...
async def get_latest_profile(table_name: str) -> DatasetProfileResponse:
    ...
    if row is None:
        raise HTTPException(
            status_code=404, detail=f"No profile found for '{table_name}'."
        )
...
async def generate_suggestions(
    ...
    if request.backend == "gemini":
        try:
            raw_suggestions = await suggest_rules_gemini(profile, request.table_name)
        ...
        raw_suggestions = suggest_rules(profile, request.table_name)

    sanitized_suggestions, rejected = sanitize_suggestions(
        raw_suggestions,
        request.table_name,
    )
    for rejection in rejected:
        log.warning(
            "Skipping suggestion '{rule}' for '{table}' — {reason}",
            rule=rejection.rule_name,
            table=request.table_name,
            reason=rejection.reason,
        )

    return await _persist_and_fetch_suggestions(sanitized_suggestions)
...
async def apply_suggestion(
    ...
    async with metadata_engine.connect() as conn:
        result = await conn.execute(
            text(
                "SELECT * FROM dq_platform.rule_suggestions WHERE suggestion_id = :id"
            ),
            {"id": suggestion_id},
        )
        row = result.mappings().first()
    ...
    if row["applied"]:
        raise HTTPException(
            status_code=409, detail="Suggestion has already been applied."
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=400,
            detail={"type": "INVALID_SUGGESTION", "message": str(exc)},
        ) from exc
    ...
    async with metadata_engine.begin() as conn:
        await conn.execute(
            text("""
                ...
                WHERE suggestion_id = :suggestion_id AND applied = FALSE
            """),
...
async def run_anomaly_detection(
    ...
    if errors and not results:
        raise HTTPException(
            status_code=400,
            detail={
                "message": "All columns failed anomaly detection.",
                "errors": errors,
            },
        )
...
async def _persist_and_fetch_suggestions(
...
def _pipeline_event_from_row(row) -> PipelineEventResponse:
...
def _schedule_response(schedule) -> PipelineScheduleResponse:
...
def _parse_json_optional(value: Any) -> dict[str, Any] | None:
    if isinstance(value, str):
        return cast(dict[str, Any], json.loads(value))
    if value is None:
        return None
...
def _profile_from_row(row) -> DatasetProfileResponse:
    return DatasetProfileResponse(
        ...
        null_summary=cast(dict[str, float], _parse_json_dict(row["null_summary"])),
        schema_info=cast(dict[str, str], _parse_json_dict(row["schema_info"])),
        statistics=_parse_json_dict(row["statistics"]),
        uniqueness=_parse_json_dict(row["uniqueness"]),
        profiled_at=row["profiled_at"],
    )
...
def _suggestion_from_row(row) -> RuleSuggestionResponse:
    return RuleSuggestionResponse(
        ...
        expected_result_value=float(row["expected_result_value"])
        if row.get("expected_result_value") is not None
        else None,
